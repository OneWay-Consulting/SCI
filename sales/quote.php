    ...
    if(isset($_POST))
		...
		if(isset($_POST['date']) && isset($_POST['date2']) || isset($_POST['client'])){
			if($_SESSION['user']->getUser()->fnidrole==7)
				$list = Quote::getAllByType($mysql,$_POST['date'], $_POST['date2'], $_POST['client'],$_SESSION['user']->getUser()->idtcuser);
			else
				$list = Quote::getAllByType($mysql,$_POST['date'], $_POST['date2'], $_POST['client']);
		}
		else{
			if($_SESSION['user']->getUser()->fnidrole==7)
				$list = Quote::getAllByType($mysql,null,null,null,$_SESSION['user']->getUser()->idtcuser);
			else
				$list = Quote::getAllByType($mysql);
		}
...
<br />
<br />
<h2><span class="label label-primary">Bandeja de Cotizaciones</span></h2>
<br />
<br />
<br />
<!-- filtros -->
...
<center>
    ...
    <form action="quotequery.php" method="post"><input type="hidden" name="dispatch" id="dispatch" value="cancel">
            <button type="button" class="btn btn-primary btn-sm" onclick="paint('quoteAddForm.php','dispatch=add');" >Capturar Cotizaci&oacute;n</button>
            <? if($_SESSION['user']->getUser()->fnidrole == 1){?>
                <button type="submit" class="btn btn-danger btn-sm">Cancelar Cotizaciones</button>
            <?}?>
        <? if($_SESSION['user']->getUser()->fnidrole != 7){?>
            <button type="button" class="btn btn-info btn-sm" onclick="location.href='uploadForm.php'" >Cargar Cotizaci&oacute;n</button>
        <?}?>
    </form>
    <? }?>
      ...
      <tbody id="myTable">
        <? for($i=0; $i<count($list); $i++){?>
                <tr id="<? echo $list[$i]->pnid;?>">
                    ...
                    <td><? if($list[$i]->dsstatus == 1 || $list[$i]->dsstatus == 4){?>
                            <img src="../images/buttons/editar.png" border="0" align="Modificar Registro" title="Modificar Registro" style="cursor:pointer; height:20px; width:20px;" onclick="sendAction('quoteAddForm.php','dispatch=update&id=<? echo $list[$i]->pnid;?>');" />
                        <? }elseif($list[$i]->dsstatus == 5){?>
                            <img src="../images/buttons/vision.png" border="0" align="Consultar Registro" title="Consultar Registro" style="cursor:pointer; height:20px; width:20px;" onclick="sendAction('quoteAddForm.php','dispatch=query&id=<? echo $list[$i]->pnid;?>');" />
                        <? }?>
                    </td>
                </tr>
                <? }?>
